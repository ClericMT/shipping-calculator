...
class Stock:
    ...
    def num_of_bundles(self): #COMPUTE BUNDLE SIZES
    ...
    def print_total_order(self): #FORMAT ORDER
...
stock_list.extend([vegemite_scroll, blueberry_muffin, croissant]) #Appends to stock list

while True: #Menu
    user_input = input('\n1. Add to order\n2. Finalise order\n3. Appendix\n')
    if user_input == '1': #ENTER CODES
        num_of_stock, item_code = input("Enter stock code: ").split()
        for item in stock_list:
            if item_code == item.code:
                item.num_of_stock += int(num_of_stock)
    elif user_input == '2': #COMPUTE ORDER
        start = time.time()
        print('OR#' + str("{0:0=8d}".format(receipt_num)))
    ...
    elif user_input == '3': #LIST STOCK CODES
        for stock in stock_list:
